...
function ManageComponents() {
  ...
  return (
    <section className={`w-full min-h-screen flex flex-col gap-8 items-center p-2 relative
     ${themeClass}`}>
      <NavBarComponent theme={themeClass}/>
...
function ThemeComponent({ theme, onClick }) {
  ...
  if(theme.trim().toLowerCase() === 'light') {
    ...
    className="size-8 md:size-8.5 fixed right-2 lg:right-16 top-8 hover:bg-black/20 rounded-full p-1 rotate-6">
    <path strokeLinecap="round" strokeLinejoin="round" d="M21.752 15.002A9.72 9.72 0 0 1 18 15.75c-5.385 0-9.75-4.365-9.75-9.75 0-1.33.266-2.597.748-3.752A9.753 9.753 0 0 0 3 11.25C3 16.635 7.365 21 12.75 21a9.753 9.753 0 0 0 9.002-5.998Z" />
    </svg>
  } else {
    content = <svg onClick={() => onClick()}
    xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" 
    className="size-8 md:size-8.5 fixed right-2 lg:right-16 top-8 hover:bg-gray-600 rounded-full p-1 rotate-6">
